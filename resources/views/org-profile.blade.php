...
@section('content')
    <div class="container">
        <div style="display: flex; padding: 20px 20px 20px 0;">
            ...
            <button class="btn-icon btn-open-modal" value="{{'update-org-'.$data->id}}"><img width="20px"
                                                                                             src="{{ URL::asset('img/pen.png') }}"
                                                                                             alt=""></button>
            <button class="btn-icon btn-open-modal" value="{{'delete-'.$data->id}}" style="margin-left: 20px"><a><img
        ...
        <x-modal-window id="{{'update-org-'.$data->id}}">
            <form id="OrgUpdate" style="margin: 20px;">
                ...
                <div class="modal-footer">
                    <button type="button" id="btnCloseUpdateModalOrg" class="btn btn-secondary btn-close-modal">
                        Закрыть
                    </button>
        ...
        @if (!empty($employees))
            <div style="display: flex;   justify-content: space-between; padding-top: 100px;">
                ...
                <button class="btn btn-success btn-open-modal" value="create-user">Добавить</button>
            </div>
            ...
            <table class="table table-striped table-sm">
                ...
                @foreach($employees as $el)
                    <tr>
                        ...
                        <td>{{$el->inn}}</td>
                        <td>{{$el->snils}}</td>
                        <td><a href="{{route('employee-data-by-id',  $el->id)}}">
                    ...
                    <x-modal-window id="{{'delete-employee-from-org-'.$el->id}}">
                        <h2 style="text-align: center; margin: 30px 0;">Удалить пользователя
                            <br/><b>{{$el->last_name}} {{$el->first_name}} {{$el->middle_name}}</b> <br/> из организации
                            <i>{{$data->name}}</i></h2>
                        <div class="modal-footer">
